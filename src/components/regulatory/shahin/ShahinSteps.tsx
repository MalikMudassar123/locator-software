...
const STEPS = [
  {
    ...
    desc: 'Vehicle registration, trade license, and Emirates ID copies ready before you start.',
    icon: (
  ...
  {
    num: '02',
    title: 'Submit via the SHAHIN Portal',
    desc: 'Apply directly through the official SHAHIN registration portal with our guidance.',
    icon: (
  ...
  {
    num: '03',
    title: 'Install a Certified GPS Device',
    desc: "Our technicians fit an approved tracker that meets SIRA's technical specification.",
    icon: (
      <svg width="24" height="24" viewBox="0 0 28 28" fill="none" aria-hidden="true">
  ...
  {
    num: '04',
    title: 'Get Certified & Compliant',
    desc: 'Receive your SHAHIN certification and stay compliant with ongoing monitoring.',
    icon: (
      <svg width="24" height="24" viewBox="0 0 28 28" fill="none" aria-hidden="true">
  ...
  return (
    <>
      <style>{`
        ...
        .shn-step-card:hover::before { opacity: 1; }

        .shn-step-grid { position: relative; display: grid; grid-template-columns: repeat(4,1fr); gap: clamp(12px,1.8vw,20px); }
        .shn-step-grid::before {
          content: '';
          position: absolute;
        ...
        @media (max-width: 800px) {
          .shn-step-grid::before { display: none; }
          .shn-step-grid { display: flex !important; flex-direction: column !important; }
        }
        ...
        .shn-portal-link {
          ...
          border: 1.5px solid #1360ee33; border-radius: 999px;
          transition: .18s ${EASE};
        }
        .shn-portal-link:hover { background: #1360ee; color: #fff; transform: translateY(-1px); }
      `}</style>

      <section id="register" style={{ padding: 'clamp(56px,7vw,80px) 28px', background: '#f7f9fc' }}>
        <div style={{ maxWidth: '1120px', margin: '0 auto' }}>
          ...
          <div data-reveal style={{ textAlign: 'center', marginBottom: '48px' }}>
            ...
            <h2 style={{
              ...
              lineHeight: 1.1, letterSpacing: '-.02em', color: '#1d1d1f', maxWidth: '540px',
            }}>
              Get SHAHIN-Certified in 4 Steps
            </h2>
            <p style={{
              margin: '16px auto 0', maxWidth: '480px',
              fontSize: 'clamp(14px,1.3vw,16px)', color: '#6e6e73', lineHeight: 1.6,
            }}>
              From paperwork to certification — our team manages the entire SHAHIN registration process for you.
            </p>
          </div>

          <div className="shn-step-grid" data-reveal>
            {STEPS.map((step, i) => (
              <div
                key={step.num}
                className="shn-step-card"
                style={{ '--step-accent': step.accent } as React.CSSProperties}
                data-reveal
                data-reveal-delay={String(i * 80)}
              >
                <span className="shn-step-num">{step.num}</span>
                <div className="shn-step-icon-wrap" style={{ background: `${step.accent}15`, color: step.accent }}>
                  {step.icon}
                </div>
          ...
          <div data-reveal style={{ textAlign: 'center', marginTop: '40px' }}>
            <a href="https://shahin.securepath.ae/" target="_blank" rel="noopener noreferrer" className="shn-portal-link">
              Visit the Official SHAHIN Portal
