...
function FMDonut({ values, colors }: { values: number[]; colors: string[] }) {
  ...
  return (
    <svg width="44" height="44" viewBox="0 0 44 44">
      ...
      {values.map((v, i) => {
        ...
        const el = <circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={colors[i]} strokeWidth={sw} strokeDasharray={`${d} ${circ - d}`} strokeDashoffset={-off} transform="rotate(-90 22 22)" />
        off += d
        return el
      })}
    </svg>
...
function FMCarSvg() {
  return (
    <svg width="36" height="20" viewBox="0 0 36 20" fill="none" stroke="#64748b" strokeWidth="1" strokeLinejoin="round">
      <rect x="2" y="10" width="32" height="5" rx="2" /><circle cx="9" cy="16" r="2.5" /><circle cx="27" cy="16" r="2.5" /><path d="M6 10l3-5h18l3 5" />
    </svg>
  )
}
...
const FMIconCal = ({ color }: { color: string }) => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5"><rect x="3" y="4" width="18" height="18" rx="2" /><path d="M16 2v4M8 2v4M3 10h18" /></svg>
const FMIconTimer = ({ color }: { color: string }) => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5"><circle cx="12" cy="13" r="8" /><path d="M12 9v4l2 2M12 5V3" /></svg>
const FMIconCheck = ({ color }: { color: string }) => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5"><circle cx="12" cy="12" r="10" /><path d="M9 12l2 2 4-4" /></svg>
const FMIconDoc = ({ color }: { color: string }) => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5"><rect x="5" y="2" width="14" height="20" rx="2" /><path d="M9 7h6M9 11h6M9 15h4" /></svg>

export default function FleetManagerShowcase() {
  const [topTab, setTopTab] = useState('reminder')
  ...
  const [hoverTopTab, setHoverTopTab] = useState<string | null>(null)

  useEffect(() => {
    setSubTab(topTab === 'reminder' ? 'service' : 'services')
    setExpandedRow(null); setSelectedRow(null); setHoverRow(null)
  }, [topTab])

  const subTabDefs = topTab === 'reminder'
    ? [{ key: 'service', label: 'SERVICE REMINDER', inBg: '#dbeafe', inColor: '#1e40af' }, { key: 'documents', label: 'DOCUMENTS REMINDER', inBg: '#dcfce7', inColor: '#166534' }]
    : [{ key: 'services', label: 'SERVICES', inBg: '#dbeafe', inColor: '#1e40af' }, { key: 'fuel', label: 'FUEL', inBg: '#dcfce7', inColor: '#166534' }, { key: 'other', label: 'OTHER EXPENSE', inBg: '#fef3c7', inColor: '#92400e' }]

  let title = '', vName = FM_VEHICLE, hType = 'reminder', tCost = 0
  let donut: { v: number[]; c: string[] } | null = null
  let stats: any[] = [], cols: string[] = [], rows: any[] = [], rActs: string[] = [], pgn = ''
  const bgM: any = { red: '#fef2f2', orange: '#fffbeb', green: '#f0fdf4' }
  const txM: any = { red: '#dc2626', orange: '#f59e0b', green: '#22c55e' }
  ...
  if (subTab === 'service') {
    ...
    rActs = ['Renew', 'Edit', 'History']; pgn = `showing 1-${FM_SERVICE_REMINDERS.length} of ${FM_SERVICE_REMINDERS.length} items`
    ...
    rActs = ['Edit', 'Attach', 'History']; pgn = `showing 1-${FM_DOC_REMINDERS.length} of ${FM_DOC_REMINDERS.length} items`
    ...
    rActs = ['Edit', 'Attach']; pgn = `showing 1-${FM_SERVICE_ENTRIES.length} of ${FM_SERVICE_ENTRIES.length} items`
  } else if (subTab === 'fuel') {
    ...
    rActs = ['Edit', 'Delete']; pgn = `showing 1-${FM_FUEL_ENTRIES.length} of ${FM_FUEL_ENTRIES.length} items`
  } else {
    ...
    rActs = ['Edit', 'Delete']; pgn = `showing 1-${FM_OTHER_EXPENSES.length} of ${FM_OTHER_EXPENSES.length} items`
  }

  const actIcon = (a: string): ReactNode => {
    ...
    <div className="ms-embed">
      ...
      <div style={{ padding: '6px 16px 4px', flexShrink: 0 }}>
        <div style={{ display: 'flex', background: '#e8edf2', borderRadius: 10, padding: 3 }}>
          {[{ k: 'reminder', l: 'Reminder', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9" /><path d="M13.73 21a2 2 0 01-3.46 0" /></svg> }, { k: 'expenditure', l: 'Fleet Expenditure', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="4" y="2" width="16" height="20" rx="2" /><path d="M8 6h8M8 10h8M8 14h4M8 18h4" /></svg> }].map(t => {
